...
- Dev commands default `TXCODE_STATE_DIR` to `~/.t3/dev` to keep dev state isolated from desktop/prod state.
...
Set `TXCODE_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: server `3773`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `TXCODE_DEV_INSTANCE`)
- Example: `TXCODE_DEV_INSTANCE=branch-a bun run dev:desktop`

If you want full control instead of hashing, set `TXCODE_PORT_OFFSET` to a numeric offset.
