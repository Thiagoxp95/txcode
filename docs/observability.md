...
```bash
export TXCODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export TXCODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export TXCODE_OTLP_SERVICE_NAME=t3-local
```

Optional:

```bash
export TXCODE_TRACE_MIN_LEVEL=Info
export TXCODE_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `TXCODE_OTLP_*`.

macOS app bundle example:

```bash
TXCODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
TXCODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
TXCODE_OTLP_SERVICE_NAME=t3-desktop \
...
```bash
tail -f "$TXCODE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$TXCODE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$TXCODE_HOME/userdata/logs/server.trace.ndjson"
```
...
- `TXCODE_OTLP_TRACES_URL` was not set
...
- `TXCODE_TRACE_FILE`: override trace file path
- `TXCODE_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `TXCODE_TRACE_MAX_FILES`: rotated file count, default `10`
- `TXCODE_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `TXCODE_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `TXCODE_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `TXCODE_OTLP_TRACES_URL`: OTLP trace endpoint
- `TXCODE_OTLP_METRICS_URL`: OTLP metric endpoint
- `TXCODE_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `TXCODE_OTLP_SERVICE_NAME`: service name, default `t3-server`
