...
it.layer(NodeServices.layer)("RepositoryIdentityResolverLive", (it) => {
  it.effect("normalizes equivalent GitHub remotes into a stable repository identity", () =>
    Effect.gen(function* () {
      ...
      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@github.com:T3Tools/txcode.git"]);

      const resolver = yield* RepositoryIdentityResolver;
      const identity = yield* resolver.resolve(cwd);

      expect(identity).not.toBeNull();
      expect(identity?.canonicalKey).toBe("github.com/t3tools/txcode");
      expect(normalizeResolvedPath(identity?.rootPath ?? "")).toBe(normalizeResolvedPath(cwd));
      expect(identity?.displayName).toBe("t3tools/txcode");
      expect(identity?.provider).toBe("github");
      expect(identity?.owner).toBe("t3tools");
      expect(identity?.name).toBe("txcode");
  ...
  it.effect("returns the git top-level root path when resolving from a nested workspace", () =>
    Effect.gen(function* () {
      ...
      yield* git(repoRoot, ["remote", "add", "origin", "git@github.com:T3Tools/txcode.git"]);
  ...
  it.effect("prefers upstream over origin when both remotes are configured", () =>
    Effect.gen(function* () {
      ...
      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@github.com:julius/txcode.git"]);
      yield* git(cwd, ["remote", "add", "upstream", "git@github.com:T3Tools/txcode.git"]);
  ...
  it.effect("uses the last remote path segment as the repository name for nested groups", () =>
    Effect.gen(function* () {
      ...
      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@gitlab.com:T3Tools/platform/txcode.git"]);

      const resolver = yield* RepositoryIdentityResolver;
      const identity = yield* resolver.resolve(cwd);

      expect(identity).not.toBeNull();
      expect(identity?.canonicalKey).toBe("gitlab.com/t3tools/platform/txcode");
      expect(identity?.displayName).toBe("t3tools/platform/txcode");
      expect(identity?.owner).toBe("t3tools");
      expect(identity?.name).toBe("txcode");
    }).pipe(Effect.provide(RepositoryIdentityResolverLive)),
  );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(refreshedIdentity?.canonicalKey).toBe("github.com/t3tools/txcode");
        expect(refreshedIdentity?.name).toBe("txcode");
      }).pipe(
        Effect.provide(
          Layer.merge(
            TestClock.layer(),
  ...
  it.effect("refreshes cached identities after the positive TTL when a remote changes", () =>
    Effect.gen(function* () {
      ...
      expect(initialIdentity?.canonicalKey).toBe("github.com/t3tools/txcode");

      yield* git(cwd, ["remote", "set-url", "origin", "git@github.com:T3Tools/txcode-next.git"]);

      const cachedIdentity = yield* resolver.resolve(cwd);
      expect(cachedIdentity).not.toBeNull();
      expect(cachedIdentity?.canonicalKey).toBe("github.com/t3tools/txcode");

      yield* TestClock.adjust(Duration.millis(180));

      const refreshedIdentity = yield* resolver.resolve(cwd);
      expect(refreshedIdentity).not.toBeNull();
      expect(refreshedIdentity?.canonicalKey).toBe("github.com/t3tools/txcode-next");
      expect(refreshedIdentity?.displayName).toBe("t3tools/txcode-next");
      expect(refreshedIdentity?.name).toBe("txcode-next");
    }).pipe(
      Effect.provide(
        Layer.merge(
          TestClock.layer(),
