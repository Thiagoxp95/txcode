...
describe("ChatView timeline estimator parity (full app)", () => {
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            worktreePath: null,
            env: {
              TXCODE_PROJECT_ROOT: "/repo/project",
            },
          });
          expect(openRequest?.env?.TXCODE_WORKTREE_PATH).toBeUndefined();
        },
        { timeout: 8_000, interval: 16 },
      );
    } finally {
  ...
  it("falls back to the first installed editor when the stored favorite is unavailable", async () => {
    localStorage.setItem("txcode:last-editor", JSON.stringify("vscodium"));
    setDraftThreadWithoutWorktree();
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            env: {
              TXCODE_PROJECT_ROOT: "/repo/project",
              TXCODE_WORKTREE_PATH: "/repo/worktrees/feature-draft",
            },
          });
        },
        { timeout: 8_000, interval: 16 },
        ...
        if (body._tag === WS_METHODS.gitResolvePullRequest) {
          return {
            pullRequest: {
              ...
              url: "https://github.com/pingdotgg/txcode/pull/1359",
              baseBranch: "main",
          ...
          expect(dispatchRequest).toMatchObject({
            ...
            bootstrap: {
              ...
              prepareWorktree: {
                ...
                baseBranch: "main",
                branch: expect.stringMatching(/^txcode\/[0-9a-f]{8}$/),
              },
              runSetupScript: true,
            },
          });
  ...
  it("shows the confirm archive action after clicking the archive button", async () => {
    ...
    try {
      ...
      localStorage.removeItem("txcode:client-settings:v1");
      await mounted.cleanup();
    }
  });
