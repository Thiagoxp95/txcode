...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("TXCODE_PORT_OFFSET"),
  devInstance: optionalStringConfig("TXCODE_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid TXCODE_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `TXCODE_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric TXCODE_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed TXCODE_DEV_INSTANCE=${seed}` };
}
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const output: NodeJS.ProcessEnv = {
      ...
      TXCODE_HOME: resolvedBaseDir,
    };

    if (!isDesktopMode) {
      output.TXCODE_PORT = String(serverPort);
      ...
      delete output.TXCODE_MODE;
      delete output.TXCODE_NO_BROWSER;
      delete output.TXCODE_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.TXCODE_HOST = host;
    }

    if (!isDesktopMode && noBrowser !== undefined) {
      output.TXCODE_NO_BROWSER = noBrowser ? "1" : "0";
    } else if (!isDesktopMode) {
      delete output.TXCODE_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.TXCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.TXCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.TXCODE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.TXCODE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.TXCODE_MODE = "web";
      delete output.TXCODE_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read TXCODE_PORT_OFFSET/TXCODE_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.TXCODE_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.TXCODE_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  t3Home: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all T3 Code data (equivalent to TXCODE_HOME)."),
    Flag.withFallbackConfig(optionalStringConfig("TXCODE_HOME")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to TXCODE_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("TXCODE_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to TXCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("TXCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to TXCODE_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("TXCODE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to TXCODE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("TXCODE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to TXCODE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("TXCODE_PORT")),
  ),
